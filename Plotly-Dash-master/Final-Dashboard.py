...
button = dbc.Button("View Data", id="open", color= "primary")

# report_button =  dbc.Button("Download Dashboard", id="report", color= "success")
...
#     # Ensure the figures are properly defined
#     if not all(isinstance(fig, go.Figure) for fig in [pie_fig, hist_fig, line_fig]):
#         return 'Error: Invalid figure objects.'

#     # Create in-memory binary streams for each chart
#     pie_stream = io.BytesIO()
#     hist_stream = io.BytesIO()
#     line_stream = io.BytesIO()
#     print(type(pio))          # Should print <class 'module'>
#     print(type(pio.write_image))  # Should print <class 'function'>
#     print(type(pie_fig))      # Should print <class 'plotly.graph_objs._figure.Figure'>


#     try:
#         # Save static images of each chart to the corresponding stream
#         pio.write_image(pie_fig, pie_stream, format='png')
#         pio.write_image(hist_fig, hist_stream, format='png')
#         pio.write_image(line_fig, line_stream, format='png')
...
#         # Create and save the final report image
...
def validate_prediction_input(country, traffic_source, user_agent):
    """
    Validate inputs for the prediction model.
    
    Args:
        country (str): Country input
        traffic_source (str): Traffic source input
        user_agent (str): User agent input
    
    Returns:
        tuple: (is_valid, error_message)
    """
    # Valid options for each input
    valid_countries = ['USA', 'United-Kingdom', 'France', 'China', 'Japan', 
                       'Canada', 'Australia', 'Spain', 'Germany', 'Italy']
    
    valid_traffic_sources = ['Facebook', 'TikTok', 'Twitter', 'Reddit']
    
    valid_user_agents = ['Windows Firefox', 'Windows Yandex', 'Smart TV - Linux x86_64',
                         'Windows Chrome', 'Windows Edge', 'Android 11 - Samsung Galaxy S20',
                         'Mac Chrome', 'Mac Safari', 'Smart TV - Linux armv7l',
                         'Android 11 - Samsung Galaxy S10', 'iOS 14 - Iphone', 'iOS 14 - Ipad', 
                         'Samsung Smart TV - Tizen 5.5', 'Samsung Smart TV - Tizen 5.0', 
                         'Android 11 - Samsung Galaxy A51']
    
    # Check if inputs are None or empty
    if not all([country, traffic_source, user_agent]):
        return False, "All fields must be filled out."
    
    # Validate each input
    if country not in valid_countries:
        return False, f"Invalid country. Please choose from: {', '.join(valid_countries)}"
    
    if traffic_source not in valid_traffic_sources:
        return False, f"Invalid traffic source. Please choose from: {', '.join(valid_traffic_sources)}"
    
    if user_agent not in valid_user_agents:
        return False, f"Invalid device/user agent. Please choose from: {', '.join(valid_user_agents)}"
...
@app.callback(
    [Output('results-badge', 'children'), Output('results-badge', 'color')],
...
def make_prediction(n_clicks, country, traffic_source, user_agent):
    # If n_clicks is None (initial load) or no input provided
    if not n_clicks or not all([country, traffic_source, user_agent]):
        return "Please fill in all fields", 'warning'
    
    # First validate the inputs
    is_valid, error_message = validate_prediction_input(country, traffic_source, user_agent)
    
    if not is_valid:
        # Return error message with danger color
        return error_message, 'danger'
    
    # If input is valid, proceed with prediction
    # encode input data
    ...
    # return prediction result with success color
    return output[0], 'primary'
